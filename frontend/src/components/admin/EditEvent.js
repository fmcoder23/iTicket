...
function EditEvent({ event, cb, categories, places }) {
    const [changes, setChanges] = useState({
        ...
        minEventPrice: event.minEventPrice,
        place: event.place,
    });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch(SummaryApi.events.put + event.id, {
                method: 'PUT',
                headers: { 'token': localStorage.getItem('token') }, // Don't set 'Content-Type' when using FormData
                body: fd
            });
            ...
            <div className='lg:w-[30%] min-w-[360px] md:w-[60%] sm:w-[60%] h-fit pb-4 bg-white rounded-xl shadow1'>
                ...
                <form onSubmit={handleSubmit}>
                    <div className='mx-8 flex flex-col gap-2'>
                        ...
                        <button type='submit' className='editButton'>Change</button>
                    </div>
                </form>
            </div>
        </div>
